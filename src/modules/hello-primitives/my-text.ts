...
const createText = async (displayText?:string) => {
  ...
  //第二个参数对应的是文字外观配置
  const textGeo = new TextGeometry(displayText||'test', {
      font: font,
      size: 3.0,
      height: .2,
      curveSegments: 12,
  ...
  const mesh = new Mesh(textGeo, createMaterial())
  ...
  textGeo.boundingBox?.getCenter(mesh.position).multiplyScalar(-1)
